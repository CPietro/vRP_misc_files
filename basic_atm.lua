    ...
    if v > 0 and v <= 2500 then
      ...
      if user_id ~= nil then
        if vRP.tryDeposit(user_id,v) then
		  ...
		  vRPclient.getPosition(player, {}, function(x,y,z) vRPca.transactionTrack({user_id, nil, "Deposito", v, "x="..x..", y="..y..", z="..z..".", nil, nil, nil}) end)
        else
      ...
      if v >= 2500 then
		vRPclient.notify(source,{"Puoi depositare al massimo 2500$"})
	  end
    ...
    if v > 0 and v <= 2500 then
      ...
      if user_id ~= nil then
        if vRP.tryWithdraw(user_id,v) then
		  ...
		  vRPclient.getPosition(player, {}, function(x,y,z) vRPca.transactionTrack({user_id, nil, "Withdrawal", v, "x="..x..", y="..y..", z="..z..".", nil, nil, nil}) end)
        else
	  ...
	  if v >= 2500 then
		vRPclient.notify(source,{"Puoi prelevare al massimo 2500$"})
	  end
      vRPclient.notify(source,{lang.common.invalid_value()})
    end
  end)
...
local function atm_carta_withdraw(player,choice)
	play_atm_enter(player)
	local user_id = vRP.getUserId(player)
	if not vRP.hasPermission(user_id,"no.withdraw") then
		vRPca.preleva({user_id})
	end	
	play_atm_exit(player)
end
...
atm_menu[lang.atm.card_withdraw.title()] = {atm_carta_withdraw,lang.atm.card_withdraw.description()}
